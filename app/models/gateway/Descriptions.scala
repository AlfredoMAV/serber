...
class Descriptions extends Table[Description]("TBL_DESCRIPTIONS") {
  ...
  def descriptionId: Column[Int] =
    column[Int]("DESCRIPTION_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = descriptionId.? ~ registryObjectKey ~ value ~ `type` ~ lang <>
    (Description.apply _, Description.unapply _)
  ...
  val registryObjects =
    foreignKey(
      "FK_TBL_DESCRIPTIONS__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  private def represent(e: Description) = {
