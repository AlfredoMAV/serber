...
class ComplexNames extends Table[ComplexName]("TBL_COMPLEX_NAMES") {
  ...
  def complexNameId: Column[Int] =
    column[Int]("COMPLEX_NAME_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column REGISTRY_OBJECT_KEY */
  def registryObjectKey: Column[Long] =
    column[Long]("REGISTRY_OBJECT_KEY", O.DBType("bigint"))

  /** Database column VALUE */
  def value: Column[Option[String]] =
    column[Option[String]]("VALUE", O.DBType("varchar(512)"))
  ...
  def lang: Column[Option[String]] =
    column[Option[String]]("LANG", O.DBType("varchar(64)"))

  /** Database default projection */
  def * = complexNameId.? ~ registryObjectKey ~ value ~ `type` ~ lang <>
    (ComplexName.interpret _, ComplexName.represent _)

  /** Database insert projection */
  def forInsert = registryObjectKey ~ value ~ `type` ~ lang <>
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_COMPLEX_NAMES__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
      r => r.registryObjectKey,
  ...
  private def represent(e: ComplexName) = {
...
object ComplexNames extends ComplexNames {
  ...
  private def complete(complexName: ComplexName) = {
