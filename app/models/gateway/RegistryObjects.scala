...
class RegistryObjects extends Table[RegistryObject]("TBL_REGISTRY_OBJECTS") {
  ...
  def registryObjectKey: Column[Long] =
    column[Long]("REGISTRY_OBJECT_KEY", O.PrimaryKey, O.DBType("bigint"), O.AutoInc)

  /** Database column REGISTRY_OBJECT_CLASS */
  def registryObjectClass: Column[Option[String]] =
    column[Option[String]]("REGISTRY_OBJECT_CLASS", O.DBType("varchar(512)"))

  /** Database column TYPE
    *
    * NOTE: The name was escaped because it collided with a Scala keyword.
    */
  def `type`: Column[Option[String]] =
    column[Option[String]]("TYPE", O.DBType("varchar(512)"))

  /** Database column ORIGINATING_SOURCE */
  def originatingSource: Column[Option[String]] =
    column[Option[String]]("ORIGINATING_SOURCE", O.DBType("varchar(512)"))

  /** Database column ORIGINATING_SOURCE_TYPE */
  def originatingSourceType: Column[Option[String]] =
    column[Option[String]]("ORIGINATING_SOURCE_TYPE", O.DBType("varchar(512)"))

  /** Database default projection */
  def * = registryObjectKey.? ~ registryObjectClass ~ `type` ~ originatingSource ~ originatingSourceType <>
    (RegistryObject.interpret _, RegistryObject.represent _)

  /** Database insert projection */
  def forInsert = registryObjectClass ~ `type` ~ originatingSource ~ originatingSourceType <>
    (interpret _, represent _)
  ...
  private def represent(e: RegistryObject) = {
    Some((
      e.registryObjectClass,
      e.`type`,
      e.originatingSource,
      e.originatingSourceType))
  }
}
...
object RegistryObjects extends RegistryObjects {
  ...
  private def complete(registryObject: RegistryObject) = {
