...
class AddressLocations extends Table[AddressLocation]("TBL_ADDRESS_LOCATIONS") {
  ...
  def addressLocationId: Column[Int] =
    column[Int]("ADDRESS_LOCATION_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column LOCATION_ID */
  def locationId: Column[Int] = column[Int]("LOCATION_ID", O.DBType("integer"))

  /** Database default projection */
  def * = addressLocationId.? ~ locationId <>
    (AddressLocation.interpret _, AddressLocation.represent _)
  ...
  def locations =
    foreignKey(
      "FK_TBL_ADDRESS_LOCATIONS__LOCATIONS",
      locationId,
      Locations)(
      r => r.locationId,
  ...
  private def interpret(locationId: Int) = {
    AddressLocation(None, locationId)
  }

  private def represent(e: AddressLocation) = {
    Some(e.locationId)
  }
}
...
object AddressLocations extends AddressLocations {
  ...
  private def complete(addressLocation: AddressLocation) = {
