...
class AddressParts extends Table[AddressPart]("TBL_ADDRESS_PARTS") {
  ...
  def addressPartId: Column[Int] =
    column[Int]("ADDRESS_PART_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column PHYSICAL_ADDRESS_ID */
  def physicalAddressId: Column[Int] =
    column[Int]("PHYSICAL_ADDRESS_ID", O.DBType("integer"))
  ...
  def * = addressPartId.? ~ physicalAddressId ~ value ~ `type` ~ lang <>
    (AddressPart.apply _, AddressPart.unapply _)

  /** Database insert projection */
  def forInsert = physicalAddressId ~ value ~ `type` ~ lang <>
  ...
  /** Foreign key referencing table PhysicalAddresses
    ...
    */
  def physicalAddresses =
    foreignKey(
      "FK_TBL_ADDRESS_PARTS__PHYSICAL_ADDRESSES",
      physicalAddressId,
      PhysicalAddresses)(
      r => r.physicalAddressId,
  ...
  private def represent(e: AddressPart) = {
