...
class Relations extends Table[Relation]("TBL_RELATIONS") {
  ...
  def relationId: Column[Int] =
    column[Int]("RELATION_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def relatedRegistryObjectKey: Column[Long] =
    column[Long]("RELATED_REGISTRY_OBJECT_KEY", O.DBType("bigint"))

  /** Database default projection */
  def * = relationId.? ~ registryObjectKey ~ relatedRegistryObjectKey <>
    (Relation.interpret _, Relation.represent _)

  /** Database insert projection */
  def forInsert = registryObjectKey ~ relatedRegistryObjectKey <>
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_RELATIONS__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  /** Foreign key referencing table RegistryObjects
    ...
    */
  def relatedRegistryObjects =
    foreignKey(
      "FK_TBL_RELATIONS__RELATED_REGISTRY_OBJECTS",
      relatedRegistryObjectKey,
      RegistryObjects)(
      r => r.registryObjectKey,
      onUpdate = ForeignKeyAction.Restrict,
      onDelete = ForeignKeyAction.Restrict)
  ...
  private def represent(e: Relation) = {
...
object Relations extends Relations {
  ...
  private def complete(relation: Relation) = {
