...
class Identifiers extends Table[Identifier]("TBL_IDENTIFIERS") {
  ...
  def identifierId: Column[Int] =
    column[Int]("IDENTIFIER_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = identifierId.? ~ registryObjectKey ~ value ~ `type` <>
    (Identifier.apply _, Identifier.unapply _)

  /** Database insert projection */
  def forInsert = registryObjectKey ~ value ~ `type` <>
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_IDENTIFIERS__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  private def represent(e: Identifier) = {
