...
class Locations extends Table[Location]("TBL_LOCATIONS") {
  ...
  def locationId: Column[Int] =
    column[Int]("LOCATION_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = locationId.? ~ registryObjectKey <>
    (Location.interpret _, Location.represent _)
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_LOCATIONS__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  private def interpret(registryObjectKey: Long) = {
    Location(None, registryObjectKey)
  }

  private def represent(e: Location) = {
...
object Locations extends Locations {
  ...
  private def complete(location: Location) = {
