...
class ElectronicAddressArgs extends Table[ElectronicAddressArg]("TBL_ELECTRONIC_ADDRESS_ARGS") {
  ...
  def electronicAddressArgId: Column[Int] =
    column[Int]("ELECTRONIC_ADDRESS_ARG_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column ELECTRONIC_ADDRESS_ID */
  def electronicAddressId: Column[Int] =
    column[Int]("ELECTRONIC_ADDRESS_ID", O.DBType("integer"))

  /** Database column NAME */
  def name: Column[Option[String]] =
    column[Option[String]]("NAME", O.DBType("varchar(512)"))

  /** Database column REQUIRED */
  def required: Column[Option[Boolean]] =
    column[Option[Boolean]]("REQUIRED", O.DBType("boolean"))
  ...
  /** Database column USE */
  def use: Column[Option[String]] =
    column[Option[String]]("USE", O.DBType("varchar(512)"))

  /** Database default projection */
  def * = electronicAddressArgId.? ~ electronicAddressId ~ name ~ required ~ `type` ~ use <>
    (ElectronicAddressArg.apply _, ElectronicAddressArg.unapply _)

  /** Database insert projection */
  def forInsert = electronicAddressId ~ name ~ required ~ `type` ~ use <>
  ...
  /** Foreign key referencing table ElectronicAddresses
    ...
    */
  def electronicAddresses =
    foreignKey(
      "FK_TBL_ELECTRONIC_ADDRESS_ARGS__ELECTRONIC_ADDRESSES",
      electronicAddressId,
      ElectronicAddresses)(
      r => r.electronicAddressId,
  ...
  private def represent(e: ElectronicAddressArg) = {
