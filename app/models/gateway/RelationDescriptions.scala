...
class RelationDescriptions extends Table[RelationDescription]("TBL_RELATION_DESCRIPTIONS") {
  ...
  def relationDescriptionId: Column[Int] =
    column[Int]("RELATION_DESCRIPTION_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column RELATION_ID */
  def relationId: Column[Int] = column[Int]("RELATION_ID", O.DBType("integer"))

  /** Database column DESCRIPTION */
  def description: Column[Option[String]] =
    column[Option[String]]("DESCRIPTION", O.DBType("varchar(512)"))
  ...
  /** Database column URL */
  def url: Column[Option[String]] =
    column[Option[String]]("URL", O.DBType("varchar(512)"))

  /** Database default projection */
  def * = relationDescriptionId.? ~ relationId ~ description ~ `type` ~ lang ~ url <>
    (RelationDescription.apply _, RelationDescription.unapply _)

  /** Database insert projection */
  def forInsert = relationId ~ description ~ `type` ~ lang ~ url <>
  ...
  def relations =
    foreignKey(
      "FK_TBL_RELATION_DESCRIPTIONS__RELATIONS",
      relationId,
      Relations)(
      r => r.relationId,
  ...
  private def represent(e: RelationDescription) = {
