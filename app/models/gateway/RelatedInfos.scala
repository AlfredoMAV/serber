...
class RelatedInfos extends Table[RelatedInfo]("TBL_RELATED_INFOS") {
  ...
  def relatedInfoId: Column[Int] =
    column[Int]("RELATED_INFO_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = relatedInfoId.? ~ registryObjectKey ~ value <>
    (RelatedInfo.apply _, RelatedInfo.unapply _)
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_RELATED_INFOS__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  private def interpret(registryObjectKey: Long, value: Option[String]) = {
    RelatedInfo(None, registryObjectKey, value)
  }

  private def represent(e: RelatedInfo) = {
