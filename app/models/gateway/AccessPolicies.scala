...
class AccessPolicies extends Table[AccessPolicy]("TBL_ACCESS_POLICIES") {
  ...
  def accessPolicyId: Column[Int] =
    column[Int]("ACCESS_POLICY_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = accessPolicyId.? ~ registryObjectKey ~ value <>
    (AccessPolicy.apply _, AccessPolicy.unapply _)
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_ACCESS_POLICIES__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  private def represent(e: AccessPolicy) = {
