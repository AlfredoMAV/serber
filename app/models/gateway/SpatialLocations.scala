...
class SpatialLocations extends Table[SpatialLocation]("TBL_SPATIAL_LOCATIONS") {
  ...
  def spatialLocationId: Column[Int] =
    column[Int]("SPATIAL_LOCATION_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column LOCATION_ID */
  def locationId: Column[Int] =
    column[Int]("LOCATION_ID", O.DBType("integer"))
  ...
  def * = spatialLocationId.? ~ locationId ~ value ~ `type` ~ lang <>
    (SpatialLocation.apply _, SpatialLocation.unapply _)

  /** Database insert projection */
  def forInsert = locationId ~ value ~ `type` ~ lang <>
  ...
  def locations =
    foreignKey(
      "FK_TBL_SPATIAL_LOCATIONS__LOCATIONS",
      locationId,
      Locations)(
      r => r.locationId,
  ...
  private def represent(e: SpatialLocation) = {
