...
class ElectronicAddresses extends Table[ElectronicAddress]("TBL_ELECTRONIC_ADDRESSES") {
  ...
  def electronicAddressId: Column[Int] =
    column[Int]("ELECTRONIC_ADDRESS_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column ADDRESS_LOCATION_ID */
  def addressLocationId: Column[Int] =
    column[Int]("ADDRESS_LOCATION_ID", O.DBType("integer"))
  ...
  def * = electronicAddressId.? ~ addressLocationId ~ value ~ `type` <>
    (ElectronicAddress.interpret _, ElectronicAddress.represent _)

  /** Database insert projection */
  def forInsert = addressLocationId ~ value ~ `type` <>
  ...
  /** Foreign key referencing table AddressLocations
    ...
    */
  def addressLocations =
    foreignKey(
      "FK_TBL_ELECTRONIC_ADDRESSES__ADDRESS_LOCATIONS",
      addressLocationId,
      AddressLocations)(
      r => r.addressLocationId,
  ...
  private def represent(e: ElectronicAddress) = {
...
object ElectronicAddresses extends ElectronicAddresses {
  ...
  private def complete(electronicAddress: ElectronicAddress) = {
