...
class Subjects extends Table[Subject]("TBL_SUBJECTS") {
  ...
  def subjectId: Column[Int] =
    column[Int]("SUBJECT_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = subjectId.? ~ registryObjectKey ~ value ~ `type` ~ lang <>
    (Subject.apply _, Subject.unapply _)
  ...
  def registryObjects =
    foreignKey(
      "FK_TBL_SUBJECTS__REGISTRY_OBJECTS",
      registryObjectKey,
      RegistryObjects)(
  ...
  private def represent(e: Subject) = {
