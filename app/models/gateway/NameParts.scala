...
class NameParts extends Table[NamePart]("TBL_NAME_PARTS") {
  ...
  def namePartId: Column[Int] =
    column[Int]("NAME_PART_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)

  /** Database column COMPLEX_NAME_ID */
  def complexNameId: Column[Int] =
    column[Int]("COMPLEX_NAME_ID", O.DBType("integer"))
  ...
  def * = namePartId.? ~ complexNameId ~ value ~ `type` <>
    (NamePart.apply _, NamePart.unapply _)
  ...
  def complexNames =
    foreignKey(
      "FK_TBL_NAME_PARTS__COMPLEX_NAMES",
      complexNameId,
      ComplexNames)(
      r => r.complexNameId,
  ...
  private def represent(e: NamePart) = {
