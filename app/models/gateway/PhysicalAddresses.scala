...
class PhysicalAddresses extends Table[PhysicalAddress]("TBL_PHYSICAL_ADDRESSES") {
  ...
  def physicalAddressId: Column[Int] =
    column[Int]("PHYSICAL_ADDRESS_ID", O.PrimaryKey, O.DBType("integer"), O.AutoInc)
  ...
  def * = physicalAddressId.? ~ addressLocationId ~ `type` ~ lang <>
    (PhysicalAddress.interpret _, PhysicalAddress.represent _)

  /** Database insert projection */
  def forInsert = addressLocationId ~ `type` ~ lang <>
  ...
  def addressLocations =
    foreignKey(
      "FK_TBL_PHYSICAL_ADDRESSES__ADDRESS_LOCATIONS",
      addressLocationId,
      AddressLocations)(
  ...
  private def represent(e: PhysicalAddress) = {
...
object PhysicalAddresses extends PhysicalAddresses {
  ...
  private def complete(physicalAddress: PhysicalAddress) = {
